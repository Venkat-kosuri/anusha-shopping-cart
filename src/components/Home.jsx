...
import { dataActions } from "../state/config/actions";

const Home = () => {
  const dispatch = useDispatch();
  const [show, setShow] = useState(false);
  const { fetchedData, cartItems } = useSelector((state) => state.data);
  const [searchValue, setSearchValue] = useState("");
  const [displayData, setDisplayData] = useState(fetchedData);
  const buttonClickedHandler = (cartItem) => {
    console.log(`cartItems`, cartItems);
    dispatch(dataActions.addITemToCart(cartItem));
  };

  useEffect(() => {
    if (displayData.length === 0) dispatch(dataActions.fetchData());
  }, []);
  ...
  useEffect(() => {
    if (searchValue === "") setDisplayData(fetchedData);
  }, [searchValue]);
              ...
              <div className="col-xs-12 col-md-3" key={item.id}>
                <div className="card">
                  ...
                  <div className="card-body">
                    <p className="card-title" style={{fontSize:'10px'}}>{item.title}</p>
                    <strong> {item.price}</strong> <br />
                    <button
                      className="btn btn-warning mt-3"
                      onClick={() => buttonClickedHandler(item)}
                    >
                      AddToCart
                    </button>
                  </div>
